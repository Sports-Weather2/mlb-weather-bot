...
def generate_recommendations(weather, impact):
    recommendations = []
    
    if impact['level'] == 'HIGH_RISK':
        if weather['rain_prob'] >= 70:
            recommendations.append("• Alert grounds crew - have tarps ready")
            recommendations.append("• Stock rain ponchos at all concession stands")
            recommendations.append("• Prepare fan communication about potential delays")
            recommendations.append("• Consider pre-game meeting with umpires")
        
        if weather['wind_gust'] >= 30:
            recommendations.append("• Secure all loose items and signage")
            recommendations.append("• Brief staff on wind safety protocols")
        
        if weather['has_thunderstorm']:
            recommendations.append("• Review lightning evacuation procedures")
            recommendations.append("• Have lightning detection system monitored")
        
        if weather['temp'] >= 100:
            recommendations.append("• Stock extra water and cooling stations")
            recommendations.append("• Brief staff on heat exhaustion signs")
        elif weather['temp'] <= 35:
            recommendations.append("• Stock hot beverages")
            recommendations.append("• Prepare heating areas for fans")
    
    elif impact['level'] == 'MONITOR':
        if weather['rain_prob'] >= 40:
            recommendations.append("• Grounds crew on standby")
            recommendations.append("• Stock rain ponchos")
        
        if weather['wind_speed'] >= 15:
            recommendations.append("• Check outdoor signage and equipment")
        
        if weather['temp'] >= 95:
            recommendations.append("• Increase water inventory")
        elif weather['temp'] <= 40:
            recommendations.append("• Increase hot beverage inventory")
    
    else:
        recommendations.append("• Normal operations")
        recommendations.append("• Standard staffing levels")
...
def format_game_block(game, weather, impact, recommendations):
    ...
    rec_text = "\n".join(recommendations) if recommendations else "• Normal operations"
...
def main():
    ...
    for game in games:
        ...
        if now <= game_datetime <= now + timedelta(hours=48):
            ...
            recommendations = generate_recommendations(weather, impact)
